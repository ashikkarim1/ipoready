  ...
  return (
    ...
    >
      ...
      <div className="divide-x divide-gray-200">
        <div className="grid grid-cols-2 gap-0">
          ...
          <div className="p-6 space-y-6">
            <div>
              <h4 className="text-lg font-bold text-gray-900 mb-2">{config1.name}</h4>
              <p className="text-xs text-gray-600">{config1.country}</p>
            </div>

            {/* Compliance Score */}
            <div className="bg-gray-50 p-4 rounded-lg">
              <p className="text-xs font-semibold text-gray-600 uppercase mb-2">Compliance Score</p>
            ...
            <div className="space-y-3">
              <p className="text-sm font-semibold text-gray-700 uppercase">Key Requirements</p>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Public Float: {config1.minPublicFloat}%</span>
                  {renderComplianceIcon(report1.gaps.some((g) => g.metric.includes('Public Float')))}
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Min Shares: {config1.minShares.toLocaleString()}</span>
                  {renderComplianceIcon(report1.gaps.some((g) => g.metric.includes('Public Shares')))}
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Min Price: ${config1.minSharePrice}</span>
                  {renderComplianceIcon(true)}
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Audit Cmte: {config1.requiresAuditCommittee ? 'Required' : 'Optional'}</span>
            ...
            <div className="bg-red-50 border border-red-200 p-4 rounded-lg">
              <p className="text-xs font-semibold text-red-900 uppercase mb-2">Issues Found</p>
          ...
          <div className="p-6 space-y-6">
            <div>
              <h4 className="text-lg font-bold text-gray-900 mb-2">{config2.name}</h4>
              <p className="text-xs text-gray-600">{config2.country}</p>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Public Float: {config2.minPublicFloat}%</span>
                  {renderComplianceIcon(report2.gaps.some((g) => g.metric.includes('Public Float')))}
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Min Shares: {config2.minShares.toLocaleString()}</span>
                  {renderComplianceIcon(report2.gaps.some((g) => g.metric.includes('Public Shares')))}
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Min Price: ${config2.minSharePrice}</span>
                  {renderComplianceIcon(true)}
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Audit Cmte: {config2.requiresAuditCommittee ? 'Required' : 'Optional'}</span>
      ...
      <div className="bg-blue-50 border-t border-gray-200 p-6">
        <h4 className="text-sm font-bold text-blue-900 mb-2">Recommendation</h4>
