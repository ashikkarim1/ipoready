...
const ipo_sequencing_1 = require("./src/lib/ipo-sequencing");
const results = {
    passed: 0,
    failed: 0,
    warnings: [],
...
if (ipo_sequencing_1.IPO_SEQUENCING_RULES.length === 17) {
    pass(`Found exactly 17 universal rules (${ipo_sequencing_1.IPO_SEQUENCING_RULES.length})`);
}
else {
    fail(`Expected 17 rules, found ${ipo_sequencing_1.IPO_SEQUENCING_RULES.length}`);
}
// Test 2: Severity distribution
log('\nTEST 2: Severity Distribution');
const errorRules = ipo_sequencing_1.IPO_SEQUENCING_RULES.filter((r) => r.severity === 'error');
const warningRules = ipo_sequencing_1.IPO_SEQUENCING_RULES.filter((r) => r.severity === 'warning');
...
log('\nTEST 5: Unique Rule IDs');
const ids = ipo_sequencing_1.IPO_SEQUENCING_RULES.map((r) => r.id);
...
for (const rule of ipo_sequencing_1.IPO_SEQUENCING_RULES) {
...
for (const example of remedationExamples) {
    const rule = ipo_sequencing_1.IPO_SEQUENCING_RULES.find((r) => r.id === example.id);
    if (rule) {
        ...
        if (keywordsFound.length === example.keywords.length) {
            pass(`${example.id} has all required keywords: ${example.keywords.join(', ')}`);
        }
        else {
            warn(`${example.id} missing keywords: ${example.keywords.filter((kw) => !fullText.includes(kw.toLowerCase())).join(', ')}`);
        }
    }
    else {
...
for (const { id, threshold } of thresholdRules) {
    const rule = ipo_sequencing_1.IPO_SEQUENCING_RULES.find((r) => r.id === id);
