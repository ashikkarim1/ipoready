-- Seed prospectus templates for all exchanges
-- This script populates the prospectus_templates table with template content for each exchange

-- Business Overview Template
INSERT INTO prospectus_templates (exchange, section_name, section_order, template_text, placeholder_fields, description, required)
VALUES 
('tsx', 'business_overview', 1, '# Business Overview
...
{key_achievements}', 
'["company_name", "incorporation_date", "incorporation_jurisdiction", "business_structure", "headquarters_address", "primary_business_description", "business_focus_areas", "products_and_services_list", "industry_sector", "market_size_estimate", "market_growth_rate", "forecast_period_years", "competitive_landscape", "competitive_advantages", "revenue_model", "revenue_streams_detail", "growth_initiatives", "expansion_plans", "key_achievements"]', 
'Company background, products/services, and market positioning', true)
ON CONFLICT (exchange, section_name) DO UPDATE SET template_text = EXCLUDED.template_text, placeholder_fields = EXCLUDED.placeholder_fields, description = EXCLUDED.description, updated_at = NOW();

-- Insert same template for all other exchanges
INSERT INTO prospectus_templates (exchange, section_name, section_order, template_text, placeholder_fields, description, required)
SELECT 
  exchange, 
  section_name, 
  ...
  required
FROM (
  SELECT DISTINCT exchange FROM prospectus_templates WHERE section_name = 'business_overview'
) exchanges,
(
  SELECT section_name, section_order, template_text, placeholder_fields, description, required
  FROM prospectus_templates WHERE section_name = 'business_overview' LIMIT 1
) template_data
WHERE exchange IN ('tsxv', 'cse', 'nasdaq', 'nyse', 'otc')
ON CONFLICT (exchange, section_name) DO NOTHING;
